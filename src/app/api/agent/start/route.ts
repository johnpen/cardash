...
import { getAccessToken, isClientCredentialsConfigured, invalidateToken } from "@/lib/auth";
...
export async function POST(req: NextRequest) {
  try {
    const { agentId, instanceEndpoint, externalSessionKey, streaming, variables } = await req.json().catch(() => ({}));
...
//    console.log('Start body' + JSON.stringify(startBody), 'info');
    const makeRequest = async (isRetry = false): Promise<Response> => {
      ...
      if (res.status === 401 && !isRetry) {
        logger.warn("Start session request unauthorized, invalidating token and retrying...");
        invalidateToken();
