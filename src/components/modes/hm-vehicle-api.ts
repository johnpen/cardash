...
import type {CarMaintenanceData, ControlMessage, TirePressure, LocationData} from '@/lib/types';
...
function createControlMessage(
  ...
  return {id, message, type, timestamp, seen};
}
...
export const getCarMaintenanceData = async (): Promise<CarMaintenanceData> => {
  try {
    ...
    const location: LocationData | null = vehicleData.vehicle_location?.coordinates ? {
        latitude: vehicleData.vehicle_location.coordinates.data.latitude,
        longitude: vehicleData.vehicle_location.coordinates.data.longitude,
        heading: vehicleData.vehicle_location.heading?.data?.value
    } : null;
